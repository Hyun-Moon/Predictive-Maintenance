...
- [x] **3단계** RUL(잔여수명) 예측 모델 — 여러 방법 비교 → [결과](docs/04_rul_model.md)
- [x] **4단계** 조건 변화 일반화 검증 (FD002/FD003/FD004) → [결과](docs/05_generalization.md)
- [x] **5단계** 실무 적용 방안 — 설비 무관 파이프라인 → [결과](docs/06_field_application.md)
...
4개 세트는 **운전조건 수와 고장모드 수가 서로 분리되어** 있습니다.
덕분에 4단계에서 "무엇이 성능을 무너뜨리는가"를 요인별로 떼어내 확인할 수 있었습니다.

### 보류: MetroPT-3 (지하철 공기압축기 실측)

실제 공기압축기 유닛에서 1초 간격으로 수집한 **실측** 데이터(약 1,516만 행).
압축기 토출압력, 오일 온도, 모터 전류 등 — **공조/유틸리티 설비와 물리적으로 거의 동일**해
실측 고유의 문제(결측·센서 고장·정비 이력)를 검증할 데이터로 계획했습니다.

**현재 환경에서 UCI 서버 접속이 불가능해 보류했습니다.**
대신 4단계는 C-MAPSS 4개 세트로 조건 변화 일반화를 검증했습니다.
데이터를 확보하면 바로 실행할 수 있도록 다운로드 스크립트와 절차는 준비되어 있습니다
...
## 6. 3단계 결과 — 잔여수명(RUL) 예측

> 재현: `python scripts/03_rul_model.py` · 전체 분석: **[docs/04_rul_model.md](docs/04_rul_model.md)**

**이 단계는 성격이 다릅니다.** 여기서는 고장 라벨을 씁니다.
따라서 회사에 바로 가져갈 것이 아니라,
**1~2단계에서 만든 특징이 실제로 유효한지 공개 벤치마크로 검증하는 단계**입니다.
...
| ① 운전시간만 | 31.15 |
| ② + 센서 원본 | 18.55 |
| ③ **+ 개체 baseline 편차** | **12.47** ← 가장 큰 도약 (33% 개선) |
| ④ + 이동평균·기울기 | 10.53 |
| ⑤ **+ 2단계 건강도 점수** | **9.07** ← 앞 단계 산출물이 재사용됨 |

1단계 EDA 에서 확인한 "개체차가 열화폭의 77%"가 **가장 큰 성능 개선으로 돌아왔습니다.**
그리고 2단계 건강도 점수를 넣자 14%가 더 개선됐습니다 — 72개 특징 중 중요도 7위입니다.
**단계마다의 산출물이 다음 단계에서 실제로 쓰이는 구조**가 확인됐습니다.

### 성능이 좋게 나와서 먼저 의심했습니다

RMSE 9.07 은 공개 논문 성적(대체로 12~16)보다 좋습니다.
**좋은 결과가 나오면 기뻐하기 전에 데이터 누수를 의심해야 합니다.**

| 검증 | 결과 |
|---|---|
| 라벨을 무작위로 섞고 재학습 | RMSE **39.63** (기준선 40.51 수준) → 누수 없음 |
| 검증 점수 vs 시험 점수 | 9.0 vs 10.5 → 시험이 약간 나쁨 = 정상 |

그럼에도 **논문과 직접 비교하지는 않습니다.** 정답 상한 처리가 논문마다 다르고,
이 프로젝트는 시험 설비의 초기 30사이클로 기준선을 잡아 정보를 더 쓰기 때문입니다.
"논문보다 좋다"가 아니라 **"이 평가 조건에서 이 숫자가 나왔다"** 로만 적었습니다.

### 오차가 어디에 몰려 있는가 — 평균보다 중요한 것

![구간별 오차](reports/figures/12_rul_error_by_band.png)

**고장이 가까울수록 정확합니다.** 잔여수명 30 이하 구간의 RMSE 는 **3.0 사이클**입니다.
100 남았을 때 12사이클 틀리는 것은 정비 계획에 영향이 없지만,
20 남았을 때 3사이클 안에 맞히면 정비 일정을 잡을 수 있습니다.

### RMSE 만 봤다면 놓쳤을 것

예측의 **49%가 실제보다 잔여수명을 길게** 보고 있었습니다 — 곧 고장을 놓치는 것입니다.
예측에서 5사이클만 빼자:
...
| 늦게 본 비율 | 49% | **28%** |
| RMSE | 9.07 | 9.77 (나빠짐) |
| PHM08 점수 | 130 | **123 (좋아짐)** |

RMSE 로만 보면 손해지만, 늦은 예측에 벌점을 2배 주는 PHM08 로 보면 이득입니다.
→ **지표 하나로 판단하면 실무적으로 틀린 결정을 하게 됩니다.**

---

## 7. 4단계 결과 — 조건이 바뀌어도 견디는가

> 재현: `python scripts/04_generalization.py` · 전체 분석: **[docs/05_generalization.md](docs/05_generalization.md)**

1~3단계는 전부 FD001(운전조건 1가지)에서 만들었습니다.
**실제 공조기는 외기온·부하율에 따라 운전점이 계속 바뀝니다.**
그래서 조건과 고장모드가 늘어난 세트에 그대로 적용해 봤습니다.

### 무너뜨린 것은 고장모드가 아니라 운전조건이었다
...
**고장 유형이 2가지로 늘어난 FD003 은 전혀 문제가 없었습니다**(오히려 더 좋음).
"정상에서 벗어난 정도"를 보는 방식이라 **어느 방향으로 벗어나는지는 상관이 없었습니다.**

무너진 것은 운전조건이 6가지인 세트뿐입니다.
원인을 재보니 **조건 간 센서값 차이가 열화 폭의 62~146배**였습니다.
...
순서가 중요합니다: **① 조건 식별 → ② 조건별 정규화 → ③ 설비별 기준선.**
②를 건너뛰면 설비 기준선이 "그 설비가 주로 어떤 조건으로 돌았는지"를 담게 됩니다.

| 세트 | 그대로 | 대책 적용 | PHM08 변화 |
|---|---:|---:|---:|
| FD002 | 15.53 | **10.68** (-31%) | 1,201 → **513** (-57%) |
| FD004 | 16.45 | **11.14** (-32%) | 1,369 → **674** (-51%) |

FD001 수준(10.0)까지 회복했고, 조건이 1가지인 세트에는 **해를 끼치지 않았습니다.**

### 실무적으로 가장 중요한 결론

![건강도 강건성](reports/figures/17_health_index_robustness.png)

라벨 없이 만든 건강도 점수는 **애초에 무너지지 않았습니다.**
그리고 단일 센서와의 격차가 조건이 늘수록 **더 벌어집니다.**

| 세트 | 센서 1개 (고정 임계값 방식) | 다변량 건강도 |
|---|---:|---:|
| FD001 (조건 1가지) | 0.756 | **0.943** |
| **FD002 (조건 6가지)** | **0.580** ← 거의 동전던지기 | **0.914** |

**운전조건이 계속 바뀌는 실제 설비일수록 고정 임계값 경보는 쓸모없어지고,
다변량 건강도 방식의 가치는 오히려 커집니다.**
"토출온도 80℃ 넘으면 알람"이 현장에서 잘 안 맞는 이유가 숫자로 확인된 셈입니다.

### 남은 한계

4개 세트 모두 **시뮬레이션 데이터**입니다.
결측치·센서 고장·정비로 인한 기준선 변화 같은 실측 고유의 문제는 아직 확인하지 못했습니다.
원래 계획했던 MetroPT-3(실제 공기압축기)는 현재 환경에서 데이터 확보가 불가능해 보류했고,
확보 시 바로 실행할 수 있도록 다운로드 스크립트와 절차를 저장소에 남겨 두었습니다.

---

## 8. 5단계 결과 — 실제 설비에 적용하기

> 재현: `python scripts/05_field_application.py` · 전체 분석: **[docs/06_field_application.md](docs/06_field_application.md)**

포트폴리오에서 가장 자주 받을 질문에 답하는 단계입니다.

> "항공기 엔진으로 했는데, 우리 공조기에도 되나요?"

말로 답하는 대신 **코드를 한 줄도 고치지 않고 공조기 데이터에 돌렸습니다.**

| | C-MAPSS (1~4단계) | 공조기 (5단계) |
|---|---|---|
| 컬럼 이름 | unit, cycle, sensor_01… | 설비번호, 일시, 필터차압… |
| 센서 | 21개 | 6개 (급기온도·차압·전류·RPM…) |
| 시간 단위 | 운전 사이클 | 1시간 |
| 운전조건 | 고도·마하수 (이산) | 외기온·부하율 (연속값) |
| 열화 | 압축기 효율 저하 | 필터 막힘 |
...
result = monitor.score(신규데이터)      # health(0~100), alarm, 원인 설명
```

![공조기 적용](reports/figures/18_ahu_health_trajectory.png)

> **이 데모의 데이터는 모의 데이터입니다.** 물리 관계를 직접 넣어 만들었으므로
> 잘 맞는 것이 당연합니다. **성능 근거가 아니라 이식 가능성 확인용**입니다.
> 성능 근거는 오직 1~4단계의 C-MAPSS 실험입니다.

### 현장에서 반드시 걸리는 함정 (전부 실제로 겪은 것)

| # | 함정 | 대응 |
|---|---|---|
| 1 | **겨울에 기준선을 잡으면 여름이 전부 이상**이 된다 | 계절 설비는 기준선이 최소 1년을 덮어야 함 |
| 2 | 정비해도 건강도가 안 올라간다 | CMMS 정비 완료 시 **기준선 자동 리셋** |
| 3 | 평활을 키우면 지속 조건이 무력해진다 | 지속 조건 ≥ 평활 창 **× 3** (코드에 검사 내장) |
| 4 | 고정 임계값은 조건이 바뀌면 무너진다 | 4단계에서 확인 — 단일 센서 AUC 0.58 |
| 5 | "점수가 낮다"만으로는 아무도 안 움직인다 | `explain()` 이 원인 센서를 σ 단위로 제시 |

### 도입 순서

```
0단계  데이터 점검         check_data() 로 사용 가능 여부 먼저 확인 (1~2주)
1단계  설비 1종 소수 대수   동일 기종 다수 + 데이터 보유 + 정지 손실 큰 것 (2~3개월)
2단계  경보 임계값 조정     오경보율 vs 리드타임 표를 만들어 관리자가 선택 (1~2개월)
3단계  CMMS 연동           경보→작업지시, 정비→기준선 리셋 (2~3개월)
4단계  확대 + RUL 도입      고장 이력이 쌓인 뒤에야 가능 (1년 이후)
```

**순서를 뒤집으면 안 됩니다.** RUL 예측은 고장 라벨이 필요한데 처음엔 그게 없습니다.
그래서 라벨이 필요 없는 건강도부터 시작합니다.
...
│   ├── 04_rul_model.md          # 3단계 RUL 예측 결과
│   ├── 05_generalization.md     # 4단계 일반화 검증 결과
│   ├── 06_field_application.md  # 5단계 현장 적용 방안
...
│   ├── 03_rul_model.py          # 3단계 RUL 예측 — 특징 누적 비교
│   ├── 04_generalization.py     # 4단계 조건 변화 일반화 검증
│   └── 05_field_application.py  # 5단계 공조기 데이터에 그대로 적용
├── src/
│   ├── config.py                # 설정 로더
│   ├── data/
...
│   │   ├── build.py             # RUL 예측용 특징 생성 (인과성 보장)
│   │   └── condition.py         # 운전조건 식별·정규화, 적응형 기준선
│   ├── models/
│   │   ├── health_index.py      # 이상탐지 5종 + 0~100 점수 변환 + 경보 규칙
│   │   └── rul.py               # RUL 모델 3종 + RMSE/PHM08 + 설비단위 분할
│   ├── deploy/
│   │   ├── pipeline.py          # 설비 종류 무관 감시 파이프라인 (현장용)
│   │   └── demo_data.py         # 공조기 모의 데이터 (이식 확인용)
│   └── viz/
│       └── style.py             # 그림 색·스타일 공통 설정
├── tests/
│   ├── test_data_load.py        # 데이터 무결성 검증 13개
│   ├── test_health_index.py     # 건강도 점수 동작 검증 20개
│   ├── test_rul.py              # 누수·분할·지표 검증 13개
│   ├── test_condition.py        # 운전조건 정규화·기준선 검증 9개
│   └── test_deploy.py           # 현장 파이프라인·경보 규칙 검증 13개
...
**성능이 너무 좋아서 의심함**
RMSE 9.07 은 공개 논문 성적보다 좋았습니다. 기뻐하기 전에 라벨을 무작위로 섞고
다시 학습시켜 봤더니 39.63(기준선 수준)으로 떨어졌습니다 — 누수 없음을 확인했습니다.
→ *성능이 예상보다 좋으면 그 자체가 점검 신호다. 라벨 셔플 테스트는 5분이면 끝난다.*

**검증 점수와 시험 점수가 섞여 있었음**
표의 시험 RMSE(10.22)와 그림의 값(10.53)이 달랐습니다. 표는 80% 학습 모델,
그림은 100% 학습 모델이었기 때문입니다.
→ *숫자마다 "어떤 데이터로 학습해 어디에 적용한 값인지"가 하나로 정해져 있어야 한다.
   0.3 차이라 넘어갈 수도 있었지만, 이런 애매함은 나중에 비교를 불가능하게 만든다.*

**건강도 점수 추가가 무의미할 줄 알았는데 아니었음**
건강도 점수는 이미 입력에 있는 특징들로 계산되는 값이라 효과가 없을 것이라 예상했는데,
RMSE 10.53 → 9.07 로 개선됐습니다.
→ *"정보가 중복되니 의미 없다"는 판단은 선형 모델 기준의 직관이다.
   트리 모델이 스스로 만들기 어려운 비선형 결합은 미리 계산해 주면 도움이 된다.*

**고장모드 때문일 것이라 예상했는데 아니었음**
조건과 고장모드가 늘어난 세트로 옮기면서 "고장 유형이 2가지로 늘어난 것"이 문제일 거라
예상했는데, FD003(고장모드 2개)은 오히려 성능이 더 좋았습니다. 무너진 건 운전조건뿐이었습니다.
→ *요인이 분리된 데이터가 있으면 추측하지 말고 나눠서 재면 된다.
   고장모드가 원인이라 믿고 모델을 복잡하게 만들었다면 시간만 버렸을 것이다.*

**기준선 30사이클이 FD001 에서만 우연히 맞았던 것**
FD002 에 관측이 21사이클뿐인 엔진이 있었습니다. 기준선이 30이면 관측 전체가 기준선이 되어
편차가 항상 0 — 모델이 영원히 건강하다고 판단합니다. FD002 6대, FD004 11대가 해당됐습니다.
FD001 은 가장 짧은 엔진이 31사이클이라 우연히 문제가 안 드러났던 것입니다.
→ *하나의 데이터에서 고른 상수는 그 데이터의 성질을 외운 것일 수 있다.
   고정값 대신 데이터에서 유도되는 규칙으로 쓴다.*

**"조건이 열화보다 1경 배 크다"는 값이 나옴**
비율 지표를 만들다 두 번 틀렸습니다. 처음엔 열화가 거의 없는 센서를 분모에 넣어 값이 폭발했고,
그걸 고치려 전체 표준편차로 필터를 걸었더니 이번엔 전부 걸러졌습니다 —
**전체 표준편차 자체가 조건 차이로 부풀려져 있었기 때문**입니다.
→ *재려는 대상이 자 안에 섞여 있으면 아무것도 못 잰다.
   조건 내 표준편차로 바꾸니 FD002 146배, FD004 62배로 정상적인 값이 나왔다.*

**건강도 점수도 무너졌을 줄 알았는데 멀쩡했음**
RUL 이 무너졌으니 건강도도 무너졌을 거라 봤는데 0.914 로 버텼고,
조건 대책을 적용해도 개선되지 않았습니다(오히려 0.907 로 미세 하락).
→ *대책은 무너진 곳에만 적용한다. "좋은 기법이니 전부 적용"은 근거 없는 습관이다.*
...
**알아챌 수 있습니다. 단, 조건이 있습니다.**

| 확인된 것 | 근거 |
|---|---|
| 고장 임박은 쉽고, **조기 탐지가 어렵다** | 단일 센서 AUC 0.994(RUL 30) vs 0.622(RUL 125) |
| 어려운 이유는 노이즈가 아니라 **개체차** | 개체 간 편차가 열화 폭의 77% |
| **고장 라벨 없이도** 건강도를 만들 수 있다 | 정상 구간만 학습해 AUC 0.943, 리드타임 114 사이클 |
| 라벨이 있으면 **잔여수명까지** 예측 가능 | RMSE 9.07, 고장 임박 구간은 3.0 |
| **운전조건 변화**가 진짜 위험 요인이다 | 조건 6가지에서 RMSE +56%, 조건별 정규화로 회복 |
| 조건이 바뀔수록 **다변량이 더 중요**해진다 | 단일 센서 0.58 vs 다변량 0.91 |
| 방법론은 **설비 종류와 무관**하다 | 코드 수정 없이 공조기 데이터에서 동작 |

### 실무로 가져갈 수 있는 것과 없는 것

| 산출물 | 필요한 데이터 | 현장 적용 |
|---|---|---|
| **건강도 점수 + 경보** | 정상 운전 데이터만 | **바로 가능** |
| RUL 예측 | 고장 이력 다수 | 이력이 쌓인 뒤 |

**이 구분이 이 프로젝트의 핵심 메시지입니다.**
벤치마크 점수보다, "고장 이력이 없는 현장에서 무엇을 할 수 있는가"가 실무적으로 더 중요합니다.

### 남은 한계

- 5개 데이터셋 모두 **시뮬레이션 또는 모의 데이터**입니다.
  결측·센서 고장·정비 이력 부정확 같은 실측 고유의 문제는 검증하지 못했습니다.
- 원래 계획했던 **MetroPT-3**(실제 공기압축기 실측)는 현재 환경에서 데이터 확보가 불가능해
  보류했습니다. 다운로드 스크립트와 적용 절차는 저장소에 준비되어 있습니다.
- 경보가 "맞았는지"를 실제 정비 이력으로 검증한 적이 없습니다.
  이것은 현장 데이터 없이는 불가능한 부분입니다.

### 만들어진 것

| | |
|---|---|
| 재현 가능한 분석 스크립트 | 5개 (`scripts/01~05`) |
| 그림 | 19장 (전부 스크립트로 재생성 가능) |
| 테스트 | **68개** (누수·분할·지표·경보 규칙 검증) |
| 기록된 실패 | **26건** ([전체](docs/99_experiments_log.md)) |
